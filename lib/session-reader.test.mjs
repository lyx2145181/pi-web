...
const { reconcileSessionFiles } = await jiti.import("./session-index-core.mts");
...
function resetSessionListState() {
  ...
  globalThis.__piSessionIndexState = undefined;
}

function installSessionIndexFake(load) {
  const coordinator = {
    invalidate() {},
    getSnapshot: load,
    forceRefresh: load,
    getVerifiedSnapshot: () => null,
  };
  globalThis.__piSessionIndexState = { version: 10, coordinator };
  return coordinator;
}
...
test("session listing reads subagent relations and terminal status without reopening full session files", async (t) => {
  ...
  resetSessionListState();
  installSessionIndexFake(async () => (await reconcileSessionFiles([filePath])).entries);
  t.after(() => {
...
test("keeps forward and reverse session path caches in sync", async () => {
  ...
  const directory = mkdtempSync(join(tmpdir(), "pi-web-cache-test-"));
...
test(
  ...
  async (t) => {
    ...
    const load = async () => {
      scans += 1;
      return (await reconcileSessionFiles([linkedPath])).entries;
    };
    installSessionIndexFake(load);
    t.after(() => {
      rmSync(externalDir, { recursive: true, force: true });
    });
...
test("falls back to the catalogue when a targeted lookup finds duplicate IDs", async (t) => {
  ...
  installSessionIndexFake(async () => {
    scans += 1;
    return (await reconcileSessionFiles([fallbackPath])).entries;
  });
...
test("stale session path cache entries are removed after external deletion", async (t) => {
  const directory = mkdtempSync(join(tmpdir(), "pi-web-stale-path-"));
  const filePath = join(directory, "session.jsonl");
  writeFileSync(filePath, "");
  resetSessionListState();
  installSessionIndexFake(async () => new Map());
  cacheSessionPath("stale-session", filePath);
  rmSync(filePath);
  t.after(() => {
    invalidateSessionPathCache("stale-session");
    resetSessionListState();
    rmSync(directory, { recursive: true, force: true });
  });

  assert.equal(await resolveSessionPath("stale-session"), null);
  assert.equal(globalThis.__piSessionPathCache?.has("stale-session"), false);
  assert.equal(globalThis.__piPathToSessionIdCache?.has(sessionPathKey(filePath)), false);
});

test("session-list invalidation forwards known paths to the incremental index", (t) => {
  resetSessionListState();
  const coordinator = installSessionIndexFake(async () => new Map());
  const invalidations = [];
  coordinator.invalidate = (paths) => invalidations.push(paths);
  t.after(resetSessionListState);

  invalidateSessionListCache(["/sessions/changed.jsonl"]);
  assert.deepEqual(invalidations, [["/sessions/changed.jsonl"]]);
});
...
test("concurrent forced session listings share one confirmed refresh", async (t) => {
  let scans = 0;
  let releaseScan;
  ...
  const first = listAllSessions({ force: true });
  const second = listAllSessions({ force: true });
...
test("reports non-sensitive session-list timing stages for a cold scan", async (t) => {
  resetSessionListState();
  installSessionIndexFake(async () => new Map());
  t.after(resetSessionListState);
  ...
  await listAllSessions({
    force: true,
    onTiming: (stage, durationMs) => stages.push([stage, durationMs]),
  });

  assert.deepEqual(stages.map(([stage]) => stage), ["session-scan", "session-map", "project"]);
