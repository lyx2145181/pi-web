...
const { AgentCommandError, isPromptBusyError, isPromptRejectedError, sendAgentCommand } = await jiti.import("./agent-client.ts");
...
test("busy prompt rejections are distinguishable for an atomic streaming retry", () => {
  const busy = new AgentCommandError("Agent is already processing", 409, "prompt_busy", false);

  assert.equal(isPromptBusyError(busy), true);
  assert.equal(isPromptRejectedError(busy), true);
  assert.equal(isPromptBusyError(new AgentCommandError("Authentication failed", 500, "prompt_rejected", false)), false);
});
