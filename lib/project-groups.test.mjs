...
test("worktree filtering keeps only sessions from the selected checkout", () => {
  const main = session("main", "/repos/project", "2026-08-12T00:00:00.000Z");
  const linked = {
    ...session("linked", "/repos/project", "2026-08-13T00:00:00.000Z"),
    cwd: "/repos/project-worktrees/feature",
  };

  assert.deepEqual(
    sessionsForWorktree([main, linked], linked.cwd).map((item) => item.id),
    ["linked"],
  );
});
