...
test("models.json reads accept BOM, comments, trailing commas and URL strings like pi", (t) => {
  const modelsPath = join(createTempRoot(t), "models.json");
  writeFileSync(modelsPath, `\uFEFF{
    // Hand-edited provider
    "providers": {
      "acme": {
        "baseUrl": "https://models.example.test/v1", // preserve // in strings
        "apiKey": "key,}",
        "models": [{ "id": "acme-2", }],
      },
    },
...
test("unreadable models.json is reported and never replaced by a save", (t) => {
  const root = createTempRoot(t);
  ...
  assert.throws(() => writeModelsConfig({ providers: {} }, modelsPath), ModelsConfigReadError);
  assert.equal(readFileSync(modelsPath, "utf8"), "[]");
});

test("empty or missing models.json can be saved, but a directory cannot be overwritten", (t) => {
  const root = createTempRoot(t);
  const modelsPath = join(root, "models.json");
  ...
  assert.throws(() => readModelsConfig(root), ModelsConfigReadError);
});
