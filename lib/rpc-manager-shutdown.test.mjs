...
test("unsubscribing during an event still delivers it to the other SSE listener", (t) => {
  const wrapper = new AgentSessionWrapper(makePromptInner(() => Promise.resolve()));
  t.after(() => wrapper.destroy());
  const received = [];
  ...
  wrapper.destroy();
  assert.deepEqual(received, ["first:session_shutdown", "second:session_shutdown"]);
});
...
test("idle shutdown waits while extension background work is active", async () => {
  let active = true;
  let disposed = false;
  const inner = makePromptInner(() => Promise.resolve());
  inner.subscribe = () => () => {};
  inner.extensionRunner = { emit: async () => {} };
  inner.dispose = () => { disposed = true; };
  const backgroundWorkProbe = {
    hasActiveWork: async () => active,
    dispose() {},
  };
  const wrapper = new AgentSessionWrapper(inner, { backgroundWorkProbe, idleTimeoutMs: 10 });
  wrapper.start();
...
test("an idle probe cannot close a session whose deadline was refreshed", async () => {
  let resolveProbe;
  let disposed = false;
  const inner = makePromptInner(() => Promise.resolve());
  inner.subscribe = () => () => {};
  inner.extensionRunner = { emit: async () => {} };
  inner.getLastAssistantText = () => undefined;
  inner.dispose = () => { disposed = true; };
  const backgroundWorkProbe = {
    hasActiveWork: () => new Promise((resolve) => { resolveProbe = resolve; }),
    dispose() {},
  };
  const wrapper = new AgentSessionWrapper(inner, { backgroundWorkProbe, idleTimeoutMs: 10 });
  wrapper.start();
  ...
  await wrapper.send({ type: "get_last_assistant_text" });
  resolveProbe(false);
  await nextTurn();
