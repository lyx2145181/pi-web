...
test("failed Basic guesses throttle even correct credentials until backoff expires", () => {
...
test("GET /api/web-auth cannot check Basic passwords without the throttle", () => {
...
test("signed cookie still works while Basic attempts are blocked", () => {
...
test("non-Basic authorization is not a password guess", () => {
  assert.equal(proxy(request("/api/sessions", { Authorization: "Bearer token" })).status, 401);
  assert.equal(getAuthRetryAfterMs(), 0);
});

test("successful Basic request does not reset a previous failure", () => {
