...
test("transient model discovery does not revive a session singleton removed during discovery", async () => {
  const existing = { token: Symbol("closing-session") };
  globalThis[SINGLETON_KEY] = existing;
...
test("transient model discovery releases a replacement acquired after session shutdown", async () => {
  const existing = { token: Symbol("closing-session") };
  const transient = { token: Symbol("model-discovery") };
  globalThis[SINGLETON_KEY] = existing;

  await runAgentServiceLoad(async () => {
    delete globalThis[SINGLETON_KEY];
    globalThis[SINGLETON_KEY] = transient;
  }, { transientExtensions: true });
