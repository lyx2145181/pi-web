...
import { closeSync, fstatSync, type Dirent, existsSync, openSync, readSync, statSync } from "fs";
...
/** Return the indexed session catalogue used for the sidebar's first paint. */
export async function listSessionSummaries(): Promise<SessionInfo[]> {
  const indexedSessions = await getIndexedSessionMetadata();
  const pathToId = new Map(indexedSessions.map((session) => [sessionPathKey(session.path), session.id]));
  const sessions = indexedSessions.map((session) => {
    cacheSessionPath(session.id, session.path);
    return {
      path: session.path,
      id: session.id,
