...
test("RPC session startup preloads extension-registered providers before restoring models", async () => {
  ...
  assert.match(
    startupSource,
    /const settingsManager = SettingsManager\.create\([\s\S]*?createPiWebAgentSessionServices\(\{[\s\S]*?settingsManager,[\s\S]*?createProjectCommandBashExtension\(\{[\s\S]*?settings: settingsManager,[\s\S]*?extensionsOverride: preferUserBashExtension/,
  );
