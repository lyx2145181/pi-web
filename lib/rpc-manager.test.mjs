...
test("RPC session startup preloads extension-registered providers before restoring models", async () => {
  ...
  assert.match(
    startupSource,
    /const settingsManager = SettingsManager\.create\([\s\S]*?createPiWebAgentSessionServices\(\{[\s\S]*?settingsManager,[\s\S]*?createProjectCommandBashExtension\(\{[\s\S]*?settings: settingsManager,[\s\S]*?extensionsOverride: \(base\) => preferUserBashExtension\(preferPiWebSubagentExtension\(base\)\)/,
  );
...
test("built-in subagents persist their selected resource policy", async () => {
  ...
  assert.match(subagentSource, /createPiWebAgentSessionServices\(/);
  assert.doesNotMatch(subagentSource, /createAgentSessionServices\(/);
...
test("normal session teardown paths use graceful extension shutdown", async () => {
  ...
  assert.match(deleteRouteSource, /await (?:getRpcSession\(id\)|runtime)\?\.shutdown\(\)/);
...
test("clone copies the requested leaf into a child session", async () => {
  ...
  assert.match(cloneSource, /invalidateParsedSession\(clonedPath\)/);
  assert.match(cloneSource, /invalidateSessionListCache\(\[clonedPath\]\)/);
...
test("exact prompts are sent through before_agent_start instead of the SDK prompt state", async () => {
  ...
  assert.match(startupSource, /\{ \.\.\.CHAT_ONLY_RESOURCE_LOADER_OPTIONS, eventBus: extensionEventBus, extensionFactories: \[exactSystemPromptExtension\] \}/);
...
test("reloading a session invalidates the models cache", async () => {
  ...
  assert.match(reloadSource, /beforeSessionStart: \(\) => \{[\s\S]*?resolveRegisteredExactToolNames\(this\.inner, activeToolNames\)/);
  assert.match(reloadSource, /await this\.inner\.reload\([\s\S]*?this\.setActiveToolSelection\(activeToolNames\);[\s\S]*?invalidateModelsCache\(\)/);
  assert.match(reloadSource, /await this\.inner\.reload\(\{/);
});

test("exact tool policy is applied after extension registration and before the first prompt", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const routeSource = await readFile(new URL("../app/api/agent/new/route.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  const createIndex = startupSource.indexOf("createAgentSessionFromServices(");
  const preBindingIndex = startupSource.indexOf("resolveRegisteredExactToolNames(", createIndex);
  const wrapperIndex = startupSource.indexOf("new AgentSessionWrapper(", preBindingIndex);
  const readyIndex = startupSource.indexOf("await wrapper.waitUntilReady()", wrapperIndex);
  const persistIndex = startupSource.indexOf("appendSessionToolSelection(", readyIndex);

  assert.match(routeSource, /validateToolActivationPolicy\(toolPolicy\)/);
  assert.match(routeSource, /toolPolicy: explicitToolPolicy/);
  assert.ok(createIndex >= 0);
  assert.ok(preBindingIndex > createIndex);
  assert.ok(wrapperIndex > preBindingIndex);
  assert.ok(readyIndex > wrapperIndex);
  assert.ok(persistIndex > readyIndex);
  assert.match(startupSource, /requestedToolPolicy === "exact"[\s\S]*?validateExactToolNames\(options\.toolNames\)/);
  assert.match(startupSource, /toolNames: selectedToolNames/);
  assert.match(startupSource, /await wrapper\.shutdown\(\);\s*throw error/);
  assert.match(source, /await bindExtensions\.call[\s\S]*?this\.setActiveToolSelection\(this\.toolNames, this\.toolPolicy\)/);
});

test("exact skill policy filters, verifies, and persists before the first prompt", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const routeSource = await readFile(new URL("../app/api/agent/new/route.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  const servicesIndex = startupSource.indexOf("createPiWebAgentSessionServices(");
  const filterIndex = startupSource.indexOf("skillsOverride:", servicesIndex);
  const verifyIndex = startupSource.indexOf("verifyExactSkillNames(", filterIndex);
  const readyIndex = startupSource.indexOf("await wrapper.waitUntilReady()", verifyIndex);
  const persistIndex = startupSource.indexOf("appendSessionSkillSelection(", readyIndex);

  assert.match(routeSource, /validateSkillActivationPolicy\(skillPolicy\)/);
  assert.match(routeSource, /skillPolicy: explicitSkillPolicy/);
  assert.match(routeSource, /skillNames/);
  assert.ok(servicesIndex >= 0);
  assert.ok(filterIndex > servicesIndex);
  assert.ok(verifyIndex > filterIndex);
  assert.ok(readyIndex > verifyIndex);
  assert.ok(persistIndex > readyIndex);
  assert.match(startupSource, /readSessionSkillSelection\(sessionManager\.getEntries\(\)/);
  assert.match(startupSource, /persistedSkillSelection\?\.skills \?\? requestedSkillNames/);
  assert.match(source, /case "get_skills"[\s\S]*?resourceLoader\.getSkills\(\)\.skills/);
});
...
test("normal sessions restore persisted tool selections before loading resources", async () => {
  ...
  assert.match(startupSource, /persistedToolSelection\?\.mode \?\? requestedToolPolicy/);
  assert.match(startupSource, /persistedToolSelection\?\.tools[\s\S]*?requestedToolNames/);
  assert.match(startupSource, /requestedToolSelectionToPersist[\s\S]*?appendSessionToolSelection\(/);
  assert.ok(startupSource.indexOf("const chatOnly") < startupSource.indexOf("createPiWebAgentSessionServices("));
  assert.match(startupSource, /chatOnly\s*\? \{ \.\.\.CHAT_ONLY_RESOURCE_LOADER_OPTIONS, eventBus: extensionEventBus, extensionFactories: \[exactSystemPromptExtension\] \}/);
...
test("crossing the Chat-only boundary persists and rebuilds the wrapper", async () => {
  ...
  assert.match(switchSource, /appendSessionToolSelection\(existing\.inner\.sessionManager, toolNames, "inclusive"\)/);
