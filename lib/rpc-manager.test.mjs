...
test("reloading a session invalidates the models cache", async () => {
  ...
  assert.match(reloadSource, /await this\.inner\.reload\(\{[\s\S]*?beforeSessionStart: \(\) => this\.setActiveToolSelection\(activeToolNames\)/);
  assert.match(reloadSource, /this\.setActiveToolSelection\(activeToolNames\);[\s\S]*?invalidateModelsCache\(\)/);
});
...
test("exact tool policy is applied after extension registration and before the first prompt", async () => {
  ...
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  const createIndex = startupSource.indexOf("createAgentSessionFromServices(");
  const activateIndex = startupSource.indexOf("resolveActiveToolNames(", createIndex);
  const wrapperIndex = startupSource.indexOf("new AgentSessionWrapper(", activateIndex);
  ...
  assert.ok(activateIndex > createIndex);
  assert.ok(wrapperIndex > activateIndex);
  ...
  assert.ok(startupSource.indexOf("resolveActiveToolNames(", createIndex) < startupSource.indexOf("appendSessionToolSelection(", createIndex));
  assert.match(startupSource, /inner\.dispose\(\);\s*throw error/);
  assert.match(startupSource, /toolPolicy,/);
});
