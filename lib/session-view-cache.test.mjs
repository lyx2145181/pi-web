...
test("a revisited session restores its own validated history, not the outgoing chat", () => {
  const old = snapshot({
  ...
  const server = { revision: "rev-1", leafId: "e1", entryIds: ["e1"] };
  assert.equal(validatedSessionViewSnapshot(old, server), old);
  assert.equal(validatedSessionViewSnapshot(old, { ...server, revision: "rev-2" }), null);
  assert.equal(validatedSessionViewSnapshot(old, { ...server, entryIds: ["other"] }), null);
  assert.equal(validatedSessionViewSnapshot(old, { ...server, leafId: "other" }), null);
  assert.equal(validatedSessionViewSnapshot({ ...old, messages: [] }, server), null);
});
