...
test("opens the selected session's event stream even when Strict Mode re-runs effects", () => {
  ...
  assert.match(mountSource, /closeEvents\(\);\s*\};[\s\S]*?\/\/ eslint-disable-next-line react-hooks\/exhaustive-deps\s*\}, \[\]\);/);
});
...
test("restoring a running session does not clear an SSE snapshot", () => {
  ...
  assert.match(sessionLoadSource, /dispatch\(\{ type: "resume" \}\)/);
  assert.doesNotMatch(sessionLoadSource, /dispatch\(\{ type: "start" \}\)/);
});
