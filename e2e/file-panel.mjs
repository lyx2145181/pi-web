...
export async function checkFilePanel(page, filePath) {
  ...
  const width = () => panel.evaluate(async (element) => {
  ...
  if (page.viewportSize().width <= 640) {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      assert.equal(await page.locator("#session-sidebar").evaluate((element) => element.inert), true);
      ...
      assert.equal(await page.locator("#session-sidebar").evaluate((element) => element.inert), false);
    }
    await toggle.click();
