...
開発サーバーはすべてのネットワークインターフェースのポート `30141` で待ち受けます。ローカルでは [http://127.0.0.1:30141](http://127.0.0.1:30141)、別の端末からはホストの LAN IP を使用してください。ループバックのみに制限するには `npm run dev:local` を実行します。一般的なチェックは次のコマンドで実行します：
